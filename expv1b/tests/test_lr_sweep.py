...
from expv1b.common import LAYERS, FISHER_LRS, add_effective_signal_metrics
from expv1b.train_expv1b import make_optimizer, private_update
from expv1b import train_expv1b as train_module
from expv1b import common
from expv1.fisher_wiener import build_fisher_state


def test_lr_grid_exact():
    assert list(FISHER_LRS) == [0.1, 0.15, 0.2, 0.3, 0.5, 0.8]
    assert list(common.RUN_SPEC_BY_ID) == [
        "dp_sgd_lr0p10", "dp_fisher_wiener_lr0p10", "dp_fisher_wiener_lr0p15",
        "dp_fisher_wiener_lr0p20", "dp_fisher_wiener_lr0p30", "dp_fisher_wiener_lr0p50",
        "dp_fisher_wiener_lr0p80",
    ]
...
def test_lr_only_optimizer(config):
    ...
    opt_a = make_optimizer(a, 0.1, config)
    opt_b = make_optimizer(b, 0.3, config)
    private_update(a, _state(a), opt_a, common.RNGStream(99, "cpu"), 2.0, config, 4,
                   "dp_fisher_wiener", 0.1, diagnostics=False)
    private_update(b, _state(b), opt_b, common.RNGStream(99, "cpu"), 2.0, config, 4,
                   "dp_fisher_wiener", 0.3, diagnostics=False)
    for old, pa, pb in zip(before, a.parameters(), b.parameters()):
        delta_a = old - pa
        delta_b = old - pb
        torch.testing.assert_close(delta_b, 3.0 * delta_a, rtol=2e-6, atol=2e-6)
...
def test_threshold_steps():
    from expv1b.common import threshold_steps
    evaluations = [

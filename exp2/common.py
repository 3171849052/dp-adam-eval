...
def parse(method=False):
    ...
    if method:
        p.add_argument('--method', required=True, choices=['dp_sgd', 'syn_diag'])
    args = p.parse_args()
...
def run_root(c):
    ...
    if not root.is_relative_to(ROOT / 'runs') or root == ROOT / 'runs':
        raise ValueError('output must be beneath exp2/runs/')
    root.mkdir(parents=True, exist_ok=True)
    return root


def datasets(c):
    # Upstream hardcodes ./data: all new cache writes go below exp2/runs/.

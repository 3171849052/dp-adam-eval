"""Small shared pieces for the paired Exp6b experiment."""
import hashlib
import json
...
METHODS = ("syn_adam_floor", "syn_adam_ema_floor", "dp_adam")
SYNTHETIC_METHODS = ("syn_adam_floor", "syn_adam_ema_floor")
...
def read_config(path):
    ...
    if set(config) != set(DEFAULT):
        raise ValueError("Config keys must match exp6b/configs/full.json")
    if config["dataset"] != "MNIST" or config["model"] != "SimpleCNN":
        raise ValueError("Exp6b uses MNIST and SimpleCNN")
    ...
    if config["weight_decay"] != 0.0:
        raise ValueError("Exp6b explicit Adam protocol requires weight_decay=0")
    if not config["smoke"]:

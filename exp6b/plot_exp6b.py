"""Plot the small set of Exp6b performance and denominator diagnostics."""
import argparse
...
from exp6b.common import METHODS, ROOT, read_config
...
def plot(config, runs, output):
    ...
    axes[1].set(xlabel="Private step", ylabel="Median effective denominator")
    for axis in axes:
        axis.grid(alpha=.2)
        axis.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(output / "exp6b.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved figure to {output / 'exp6b.png'}")

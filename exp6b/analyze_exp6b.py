"""Write paired Exp6b endpoint comparisons."""
import argparse
import json
...
from exp6b.common import METHODS, ROOT, fingerprint, read_config, save_json

CONTRASTS = (("syn_adam_floor", "dp_adam"),
             ("syn_adam_ema_floor", "syn_adam_floor"),
             ("syn_adam_ema_floor", "dp_adam"))
METRICS = ("final_accuracy", "best_accuracy", "late_mean_accuracy",
           "final_test_loss", "epsilon_spent", "final_update_norm",
           "final_first_moment_norm", "final_denominator_q05",
           "final_denominator_q50", "final_denominator_q95",
           "final_max_inverse_denominator")
...
def analyze(config, runs, output):
    ...
    for method in METHODS:
        ...
        for metric in METRICS:
            ...
            across.append(dict(method=method, metric=metric,
                               ...
                               n=len(values)))
    pd.DataFrame(across).to_csv(output / "summary_across_seeds.csv", index=False)
    ...
    lines = ["# Exp6b paired comparisons", "",
...
def main():
    ...
    analyze(read_config(args.config), args.runs, args.output or args.runs)

"""Shared Exp5 protocol, deterministic streams, and audit helpers."""
import csv
import hashlib
import json
import os
...
METHODS = ("syn_diag", "syn_diag_beta1", "syn_diag_beta2", "syn_diag_beta12", "dp_adam", "dp_kfc_adam")
SYNTHETIC_METHODS = ("syn_diag", "syn_diag_beta1", "syn_diag_beta2", "syn_diag_beta12", "dp_kfc_adam")
LAYERS = ("conv1", "conv2", "fc1", "fc2")

DEFAULT = dict(
    dataset="MNIST", model="SimpleCNN", seeds=[42, 7, 91], learning_rate=.001,
    batch_size=256, epochs=5, max_grad_norm=1., epsilon=1., delta=1e-5,
    beta1=.9, beta2=.999, adam_eps=1e-8, weight_decay=0., M_syn=2560, K=50,
    **{"lambda": 1e-3}, damping=1e-3, device="auto", threads=4,
    analysis_batch_size=32, eval_interval=100, diagnostic_interval=50,
    eps_num=1e-12, gram_chunk=2048, M_oracle=512, M_stale=512,
    stale_seed_offset=100003, oracle_seed=314159, oracle_enabled=True,
...
def read_config(path):
    ...
    if c["smoke"]:
        flexible |= {"smoke", "seeds", "learning_rate", "batch_size", "epochs", "K", "M_syn", "M_oracle",
                     "M_stale", "train_subset", "test_subset", "eval_interval", "diagnostic_interval"}
    for key in set(DEFAULT) - flexible:
        if c[key] != DEFAULT[key]:
            raise ValueError(f"Fixed protocol mismatch: {key}")
    for key in ("batch_size", "epochs", "K", "M_syn", "M_oracle", "M_stale", "threads", "analysis_batch_size",
                "eval_interval", "diagnostic_interval", "gram_chunk"):

...
from exp6.common import DEFAULT, METHODS
from exp6.train_exp6 import train
...
def test_three_methods_share_private_and_synthetic_streams(tmp_path):
    ...
    synthetic = [(item["samples_hash"], item["labels_hash"])
                 for item in audits["syn_adam"]["synthetic"]]
    assert [(item["samples_hash"], item["labels_hash"])
            for item in audits["syn_adam_ema"]["synthetic"]] == synthetic

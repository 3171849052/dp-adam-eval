"""Train the three paired Exp6 methods."""
import argparse
import copy
import json
import time
...
from exp6.common import (DEFAULT, METHODS, ROOT, SYNTHETIC_METHODS, Indexed,
...
from exp6.optimizers import FirstMomentState, SecondMomentState, ExplicitAdamState
...
def synthetic_q(state, samples, config, device):
    ...
    The diagnostic model is disposable.  In particular, this function never
    receives or writes the private model's ``grad_sample`` tensors.
    """
    x, y = samples
...
def private_update(model, noise_rng, sigma, config, batch_size):
    ...
    No synthetic state is an input here.  Thus every method clips exactly the
    private ``grad_sample`` produced by the current batch before adding noise.
    """
...
def _refresh_metrics(q, config, previous_q, step):
    rows = []
    for name, value in q.items():
        ...
        row["denominator_q05"] = float((sqrt_q + config["adam_eps"]).double().quantile(.05))
        row["denominator_q50"] = float((sqrt_q + config["adam_eps"]).double().quantile(.50))
        row["denominator_q95"] = float((sqrt_q + config["adam_eps"]).double().quantile(.95))
        row["max_inverse_denominator"] = float(1.0 / (sqrt_q + config["adam_eps"]).double().min())
        rows.append(row)
    return rows
...
def _direction_state(method, model, config):
    ...
    return FirstMomentState(list(model.parameters()), config["beta1"]), (
        SecondMomentState(config["beta2"]) if method == "syn_adam_ema" else None)
...
def train(config, seed, method, output, data_override=None, event_hook=None):
    ...
    meta = dict(seed=seed, method=method, fingerprint=fingerprint(config),
                ...
                rng_seeds=dict(init=seed, loader=seed + 1, synthetic=None if syn_rng is None else seed + 3,
                               noise=seed + 4),
    ...
    try:
        for epoch in range(config["epochs"]):
            ...
            for _ in range(len(loader)):
                ...
                if method in SYNTHETIC_METHODS and step % config["K"] == 0:
                    ...
                    refresh_record = _refresh_metrics(current_q, config, previous_q, step)
                ...
                if method == "dp_adam":
                    ...
                    if method == "syn_adam":
                        v_hat = {name: value.detach() for name, value in source_q.items()}
                    else:
                        v_hat = second_state.update(source_q)
                    denominator_map = {name: value.sqrt().to(m_hat[index].dtype).add(config["adam_eps"])
                                       for index, (name, value) in enumerate(v_hat.items())}
                    direction = [m / denominator_map[name] for m, name in zip(m_hat, denominator_map)]
                ...
                denominator_stats = _parameter_stats(denominator_values, "denominator_", include_inverse=True)
                second_stats = _parameter_stats(list(v_hat.values()), "second_moment_")
                q_distance = None
                if method == "syn_adam_ema":
                ...
                row = dict(seed=seed, method=method, step=step, epoch=epoch + 1,
                           ...
                           **{key: value for key, value in mechanism.items()
                              if key not in ("clean", "noisy", "noise_rng_before", "noise_rng_after", "noise_hash")},
        ...
        summary = dict(status="completed", seed=seed, method=method,
                       ...
                       epsilon_spent=accountant.get_epsilon(config["delta"]),
                       noise_multiplier=sigma, total_time=elapsed,
                       final_update_norm=rows[-1]["update_norm"],
                       final_first_moment_norm=rows[-1]["first_moment_norm"],

...
def _seed_metrics(trajectories, geometry, energy_summary, geometry_summary, runs):
    rows = []
    for run in runs:
...
def analyze(source=DEFAULT_SOURCE, output=DEFAULT_RESULTS):
    ...
    window_by_seed = pd.concat([energy_summary, geometry_summary, geometry_auc], ignore_index=True)
    window_mean_std = across_seed_mean_std(window_by_seed)
    seed_metrics = _seed_metrics(trajectories, geometries, energy_summary,
                                 pd.concat([geometry_summary, geometry_auc], ignore_index=True), bundle["runs"])
    ...
    correlations = correlation_summary(
        trajectories,
        ("fc_share", "fc_to_conv", "layer_entropy"),
        ("coefficient_cv", "aggregate_cosine", "clipping_shape_error", "clipping_alpha_star",
         "clipped_aggregate_norm", "diagnostic_snr"),
    )
